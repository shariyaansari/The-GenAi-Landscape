...
def run_script(script_path):
    """Helper to run a script and check if it exists"""
...
def main():
    ...
    # These paths are now relative to the 'backend' folder
    scrapers = [
        os.path.join(BASE_DIR, "scraping", "FuturePedia", "scrape_all_and_save.py"),
        os.path.join(BASE_DIR, "scraping", "product_hunt", "scrape_all_products.py"), 
        os.path.join(BASE_DIR, "scraping", "huggingface", "scrape_all_models.py")
    ]
    ...
    json_paths = [
        ...
        os.path.join(BASE_DIR, "scraping", "product_hunt", "product_hunt_data.json"),
        os.path.join(BASE_DIR, "scraping", "huggingface", "huggingface_data.json")
    ]
    ...
    # 5. MONGODB UPLOAD
    if not all_tools:
        print("⚠️ No tools collected. Skipping database update.")
        return

    mongo_uri = os.getenv("MONGO_URI")
    if not mongo_uri:
        print("❌ MONGO_URI missing. Cannot upload.")
        return
